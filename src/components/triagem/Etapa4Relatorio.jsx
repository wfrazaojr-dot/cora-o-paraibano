...
export default function Etapa4Relatorio({ dadosPaciente, onAnterior, pacienteId, modoLeitura = false }) {
  ...
  const infoTransporte = dadosPaciente.avaliacao_clinica?.info_transporte || {};
  ...
  const gerarPDF = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `Relatorio_${dadosPaciente.nome_completo?.replace(/ /g, "_")}_${format(new Date(), "yyyyMMdd_HHmm")}.pdf`;
      ...
      <div ref={relatorioRef} className="bg-white border-2 border-gray-300 rounded-lg p-4 shadow-lg" style={{ maxWidth: "210mm" }}>
        <div className="mb-4">
          <div className="flex items-center justify-between gap-3 w-full mb-3">
            ...
            <img src="https://media.base44.com/images/public/68fa0edee56f5a67f929da76/d2078127c_LOGOCARDIOPB.jpg" alt="CARDIOPB" className="h-12 w-auto object-contain" crossOrigin="anonymous" />
        ...
        <div className="mb-4">
          ...
          {dadosPaciente.avaliacao_clinica?.heart_score?.total > 0 && (
            <div className="mt-2 bg-blue-50 border border-blue-500 rounded p-2">
              <p className="font-bold text-xs">HEART SCORE: {dadosPaciente.avaliacao_clinica.heart_score.total} pontos - {dadosPaciente.avaliacao_clinica.heart_score.interpretacao}</p>
            </div>
          )}
        </div>
