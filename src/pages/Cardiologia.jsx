...
import { ArrowLeft, Heart, Download, User, Stethoscope, CheckCircle2 } from "lucide-react";
...
export default function Cardiologia() {
  ...
  const salvarAssessoriaMutation = useMutation({
    mutationFn: async (dados) => {
      return base44.entities.Paciente.update(pacienteId, {
        assessoria_cardiologia: dados
      });
    },
    onSuccess: () => {
  ...
  const handleSalvarParecer = async () => {
    const assessoria = paciente?.assessoria_cardiologia || {};
    ...
    if (tipoProfissional === "medico") {
      dadosAtualizados.cardiologista_nome = profissional.nome;
      dadosAtualizados.cardiologista_crm = profissional.registro;
      dadosAtualizados.parecer_cardiologista = parecerMedico;
    } else {
      dadosAtualizados.enfermeiro_nome = profissional.nome;
      dadosAtualizados.enfermeiro_coren = profissional.registro;
      dadosAtualizados.parecer_enfermeiro = parecerEnfermeiro;
    }

    await salvarAssessoriaMutation.mutateAsync(dadosAtualizados);
    setEtapa("relatorio");
  };
  ...
  const gerarPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Assessoria_Cardiologia_${paciente?.nome_completo?.replace(/ /g, "_")}_${format(new Date(), "yyyyMMdd_HHmm")}.pdf`);
    } catch (error) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-purple-900 flex items-center gap-3">
            ...
            Assessoria de Cardiologia
          </h1>
          <p className="text-gray-600 mt-2">Parecer técnico especializado</p>
        </div>
        ...
        {etapa === "identificacao" && (
          <Card className="shadow-lg">
            <CardHeader className="bg-purple-50 border-b">
              <CardTitle className="text-purple-900">Identificação do Profissional</CardTitle>
            </CardHeader>
          ...
          <div className="space-y-6">
            ...
            <Card className="shadow-lg border-l-4 border-l-blue-600">
              <CardHeader className="bg-blue-50">
                <CardTitle className="text-blue-900">Dados do Paciente</CardTitle>
              </CardHeader>
            ...
            {tipoProfissional === "enfermeiro" && paciente.assessoria_cardiologia?.parecer_cardiologista && (
              <Card className="shadow-lg border-l-4 border-l-green-600">
                <CardHeader className="bg-green-50">
                  <CardTitle className="text-green-900">Parecer do Médico Cardiologista</CardTitle>
                </CardHeader>
                <CardContent className="p-6">
                  <p className="text-sm font-semibold mb-2">
                    {paciente.assessoria_cardiologia.cardiologista_nome} - CRM: {paciente.assessoria_cardiologia.cardiologista_crm}
                  </p>
                  <p className="text-gray-700 whitespace-pre-wrap">{paciente.assessoria_cardiologia.parecer_cardiologista}</p>
                </CardContent>
              </Card>
            )}
            ...
            <Card className="shadow-lg border-l-4 border-l-purple-600">
              <CardHeader className="bg-purple-50">
                <CardTitle className="text-purple-900">
                  {tipoProfissional === "medico" ? "Parecer do Médico Cardiologista" : "Parecer do Enfermeiro Assessor"}
                </CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Digite o parecer técnico e a conduta recomendada..."
                    rows={10}
                    className="resize-y"
                  />
                </div>
          ...
          <div className="space-y-6">
            ...
            <div ref={relatorioRef} className="bg-white border-2 border-gray-300 rounded-lg p-8 shadow-lg">
              ...
              <div className="mb-6">
                ...
                <div className="text-center pb-4 border-b-4 border-purple-600">
                  <h1 className="text-3xl font-bold text-purple-900">RELATÓRIO DE ASSESSORIA DE CARDIOLOGIA</h1>
                  <p className="text-gray-600 mt-2">{format(new Date(), "dd 'de' MMMM 'de' yyyy 'às' HH:mm", { locale: ptBR })}</p>
                </div>
              </div>
              ...
              {paciente.assessoria_cardiologia?.parecer_cardiologista && (
                <div className="mb-6">
                  <h2 className="text-xl font-bold text-gray-900 mb-3 pb-2 border-b-2 border-gray-300">PARECER DO MÉDICO CARDIOLOGISTA</h2>
                  <div className="bg-green-50 border-2 border-green-500 rounded-lg p-4 mb-3">
                    <p className="font-semibold text-sm mb-2">
                      {paciente.assessoria_cardiologia.cardiologista_nome} • CRM: {paciente.assessoria_cardiologia.cardiologista_crm}
                    </p>
                    <p className="text-sm whitespace-pre-wrap">{paciente.assessoria_cardiologia.parecer_cardiologista}</p>
                  </div>
                </div>
              )}

              {/* Parecer do Enfermeiro */}
              {paciente.assessoria_cardiologia?.parecer_enfermeiro && (
                <div className="mb-6">
                  <h2 className="text-xl font-bold text-gray-900 mb-3 pb-2 border-b-2 border-gray-300">PARECER DO ENFERMEIRO ASSESSOR</h2>
                  <div className="bg-blue-50 border-2 border-blue-500 rounded-lg p-4 mb-3">
                    ...
                    <p className="text-sm whitespace-pre-wrap">{paciente.assessoria_cardiologia.parecer_enfermeiro}</p>
                  </div>
                </div>
              )}
